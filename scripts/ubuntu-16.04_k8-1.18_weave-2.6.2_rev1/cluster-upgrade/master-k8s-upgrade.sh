...
echo 'upgrading packages to: kubeadm=1.18.2-00'
...
echo 'upgrading kubeadm to v1.18.2'
...
sleep 120
kubeadm upgrade apply v1.18.2 -y

echo 'upgrading packages to: kubelet=1.18.2-00 kubectl=1.18.2-00 kubernetes-cni=0.7.5-00'
apt-get -q update -o Acquire::Retries=3 -o Acquire::http::No-Cache=True -o Acquire::http::Timeout=30 -o Acquire::https::No-Cache=True -o Acquire::https::Timeout=30 -o Acquire::ftp::Timeout=30
apt-get install -y --allow-change-held-packages kubelet=1.18.2-00 kubectl=1.18.2-00 kubernetes-cni=0.7.5-00 --allow-unauthenticated
